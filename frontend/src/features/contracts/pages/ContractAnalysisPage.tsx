...
export default function ContractAnalysisPage() {
  ...
  const { data, isLoading, isError, error, refetch } =
    useContractAnalysis(documentId);

  if (!documentId) {
    return (
      <EmptyState
        title="Select a document to analyze"
        description="Choose a document from your library or upload a new contract to view its risk analysis."
  ...
  if (isLoading) return <ContractAnalysisSkeleton />;
  ...
  if (!data?.analysis) {
    return (
      <EmptyState
        ...
        description="Run a fresh analysis by uploading the document again."
        actionLabel="Analyze new contract"
  ...
  const radarData = analysis.riskBreakdown?.length
    ? analysis.riskBreakdown.map((item) => ({
  ...
  const financialRisks = analysis.financialRisks?.length
    ? analysis.financialRisks
    : [];
    ...
    <div style={{ padding: "32px 28px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ color: "#64748b", fontSize: 15 }}>
              AI-powered analysis of {fileLabel}
            </p>
          </div>
          <button
      ...
      >
        {[
          ...
          {
            label: "AI Confidence",
            value: aiConfidence ? `${Math.round(aiConfidence)}%` : "—",
            color: "#6366f1",
            icon: Shield,
            desc: "Analysis accuracy",
          },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
      ...
      >
        ...
        >
          ...
          >
            Higher score = more favorable for tenant
          </p>
        </motion.div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {financialRisks.length ? (
              ...
              <div style={{ color: "#64748b", fontSize: 13 }}>
                No financial risk data was extracted from this contract.
              </div>
            )}
          </div>
        </motion.div>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {clauses.length ? (
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p
                        style={{
                          ...
                          color: "#94a3b8",
                          lineHeight: 1.6,
                          marginBottom: clause.safer ? 10 : 0,
                        }}
                      >
                        {clause.explanation}
                      </p>
            ...
            <div style={{ color: "#64748b", fontSize: 13 }}>
              No clause risks were detected for this contract.
            </div>
          )}
        </div>
      </motion.div>
      ...
      >
        <p style={{ fontSize: 12, color: "#fbbf24", lineHeight: 1.6, margin: 0 }}>
          ⚠️ This analysis is for educational purposes only and does not
          constitute legal advice. AI analysis may not capture all legal
          nuances. Consult a licensed Ethiopian attorney before signing any
          legal document.
        </p>
      </div>
    </div>
